using Distances
function  e1(StartPoint, GradientVolume, StepSize)
    ...
    xBas=[0 0 1 1].+int(floor(StartPoint[1]))
    yBas=[0 1 0 1].+int(floor(StartPoint[2]))
    xCom=StartPoint[1].-int(floor(StartPoint[1]) )
    yCom=StartPoint[2].-int(floor(StartPoint[2]) )
    ...
    xBas=min(maximum(xBas,1),size(Gx,1))
    yBas=min(maximum(yBas,1),size(Gx,2))
